...
public class DB {
	...
	public static void setClientManifestBatch(String paths[], String clientHashes[]) {
		...
		if (paths.length == clientHashes.length) try {
			...
			OpenFeintInternal.log("ForTest", "finishItem::succecd to update local manifest 1" );
        } catch (SQLiteDiskIOException e) {
        	OpenFeintInternal.log("ForTest", "finishItem::failed to update local manifest 1" );
            WebViewCache.diskError();
        } catch (SQLException e) {
        	OpenFeintInternal.log("ForTest", "finishItem:: failed to update local manifest 2" );
			OpenFeintInternal.log(TAG, e.toString());
		} finally {
			try { db.endTransaction(); } catch (Exception e) {OpenFeintInternal.log("ForTest", "finishItem:: failed to update local manifest 3" );}
		}
	}
	...
	public static void setClientManifest(Map<String, String> allData) {
		try {
			SQLiteDatabase db = storeHelper.getWritableDatabase();
			db.beginTransaction();
			try {
				for( String strpath : allData.keySet() )
				{
					String strHash = allData.get(strpath);
					db.execSQL("INSERT OR REPLACE INTO manifest(path, hash) VALUES(?,?)", new String[] {strpath, strHash});
				}
				db.setTransactionSuccessful();
			}catch (SQLiteDiskIOException e) {
	            WebViewCache.diskError();
	...
	private static final String DBPATH = "/the9GameCenter/webui/manifest.db";
