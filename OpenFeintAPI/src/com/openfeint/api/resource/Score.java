...
public class Score extends Resource {
	...
	private void submitToInternal(final Leaderboard leaderboard, final String timestamp, final SubmitToCB cb, final boolean fromOffline) {
		...
		args.put("high_score[challenge_result]", this.challengeResult);
		...
		JSONRequest req = new JSONRequest(args) {
			...
			@Override protected void onResponse(int responseCode, Object responseBody) {	
				if (201 == responseCode) {
					Score s = (Score)responseBody;
					if(s.challengeResult.equals("") == false){
					  SimpleNotification.show(s.challengeResult, "@drawable/of_icon_challenge_notification", Notification.Category.HighScore, Notification.Type.Success);
					}
					if (!fromOffline) {
...
//					Score s = scores.get(0);
	...
	public static ResourceClass getResourceClass() {
		...
		klass.mProperties.put("challenge_result", new StringResourceProperty() { public void set(Resource obj, String val) { ((Score)obj).challengeResult = val; } public String get(Resource obj) { return ((Score)obj).challengeResult; } });
