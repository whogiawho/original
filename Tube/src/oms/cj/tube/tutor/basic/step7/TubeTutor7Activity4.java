...
public class TubeTutor7Activity4 extends TubeTutorBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		View view = AdGlobals.getInstance().inflateContentView(this, R.layout.practise6, AdGlobals.getInstance().getAdInterface());
		setContentView(view);
        ...
        if(AdGlobals.getInstance().the9Switch){
            SHI_BA_PAN_Callback cb = new SHI_BA_PAN_Callback(vQuiz); 
	...
	class SHI_BA_PAN_Callback implements ITubeRenderCallbacks {
		TutorQuizView mVQuiz;
		
		SHI_BA_PAN_Callback(TutorQuizView vQuiz){
			mVQuiz = vQuiz;
		}

		private boolean isInPlace(Tube t){
			boolean bInPlace = isTutorStep7InPlace(t);
			
			return bInPlace;
		}
		@Override
		public void onRotateFinish(RotateAction r) {
			Log.i(TAG+".SHI_BA_PAN_Callback.onRotateFinish", "being called!");
	        Tube t = mVQuiz.getRenderer().getTube();
	        
	        boolean bInPlace = isInPlace(t);
	        
	        if(bInPlace){
	        	Log.i(TAG+".SHI_BA_PAN_Callback.onRotateFinish", "pls unlock SHI_BA_PAN!");
				final Achievement a = new Achievement(TubeApplication.cjAchievements[TubeApplication.SHI_BA_PAN]);
				Achievement.LoadCB cb = new TubeTutorLoadCB(a, TubeTutor7Activity4.this);
				a.load(cb);	        	
	        }
		}
