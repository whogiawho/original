...
public class TubeTutor4Activity4 extends TubeTutorBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		View view = AdGlobals.getInstance().inflateContentView(this, R.layout.practise3, AdGlobals.getInstance().getAdInterface());
		setContentView(view);
        ...
        if(AdGlobals.getInstance().the9Switch){
            YAO_WANG_DIAN_Callback cb = new YAO_WANG_DIAN_Callback(vQuiz); 
	...
	class YAO_WANG_DIAN_Callback implements ITubeRenderCallbacks {
		TutorQuizView mVQuiz;
		
		YAO_WANG_DIAN_Callback(TutorQuizView vQuiz){
			mVQuiz = vQuiz;
		}

		private boolean isInPlace(Tube t){
			boolean bInPlace = isTutorStep4InPlace(t);
			
			return bInPlace;
		}
		@Override
		public void onRotateFinish(RotateAction r) {
			Log.i(TAG+".YAO_WANG_DIAN_Callback.onRotateFinish", "being called!");
	        Tube t = mVQuiz.getRenderer().getTube();
	        
	        boolean bInPlace = isInPlace(t);
	        
	        if(bInPlace){
	        	Log.i(TAG+".YAO_WANG_DIAN_Callback.onRotateFinish", "pls unlock YAO_WANG_DIAN!");
				final Achievement a = new Achievement(TubeApplication.cjAchievements[TubeApplication.YAO_WANG_DIAN]);
				Achievement.LoadCB cb = new TubeTutorLoadCB(a, TubeTutor4Activity4.this);
				a.load(cb);	        	
	        }
		}
