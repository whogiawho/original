...
public class TubeTutor9Activity4 extends TubeTutorBaseActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		View view = AdGlobals.getInstance().inflateContentView(this, R.layout.practise8, AdGlobals.getInstance().getAdInterface());
		setContentView(view);
        ...
        if(AdGlobals.getInstance().the9Switch){
            YU_HUANG_DING_Callback cb = new YU_HUANG_DING_Callback(vQuiz); 
	...
	class YU_HUANG_DING_Callback implements ITubeRenderCallbacks {
		TutorQuizView mVQuiz;
		
		YU_HUANG_DING_Callback(TutorQuizView vQuiz){
			mVQuiz = vQuiz;
		}

		private boolean isInPlace(Tube t){
			boolean bInPlace = isTutorStep9InPlace(t);
			
			return bInPlace;
		}
		@Override
		public void onRotateFinish(RotateAction r) {
			Log.i(TAG+".YU_HUANG_DING_Callback.onRotateFinish", "being called!");
	        Tube t = mVQuiz.getRenderer().getTube();
	        
	        boolean bInPlace = isInPlace(t);
	        
	        if(bInPlace){
	        	Log.i(TAG+".YU_HUANG_DING_Callback.onRotateFinish", "pls unlock YU_HUANG_DING!");
				final Achievement a = new Achievement(TubeApplication.cjAchievements[TubeApplication.YU_HUANG_DING]);
				Achievement.LoadCB cb = new TubeTutorLoadCB(a, TubeTutor9Activity4.this);
				a.load(cb);	        	
	        }
		}
